"""Dataset smoke check for a target domain — sanity check that dataset paths
...
def collect_domain_images(domain: str, datasets_root: Path, limit: Optional[int] = None) -> List[str]:
    ...
    if not root.exists():
        print(f"[domain_dataset_smoke] WARNING: {root} does not exist — dataset not converted/placed yet?")
        return []
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Dataset path smoke check (not a performance benchmark)")
