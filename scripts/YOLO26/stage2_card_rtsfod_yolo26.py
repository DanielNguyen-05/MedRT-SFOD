"""Stage 2 RT-SFOD + reliability-guided compression (CARD-v2) for YOLO26.

The RT-SFOD baseline (Mean Teacher + DHF + MARD) is kept unchanged.  CARD-v2
compresses only the *student forward path*: structured feature-channel gates
and weight fake-QAT are scheduled by pseudo-label reliability.  Importantly,
the student's latent parameters are never zero-baked, so epoch-level EMA still
updates a genuinely dense/full-precision teacher.

This file intentionally does NOT claim a physically smaller/faster checkpoint.
The saved YOLO checkpoint contains dense latent weights and is accompanied by a
``*.card_state.pt`` file that reconstructs the training-time compression state.
Graph-aware physical channel removal + integer export is a separate deployment
step and must be benchmarked before making latency/model-size claims.
"""
...
from scripts.YOLO26.card_compression import CardControllerV2 as CardController, add_card_v2_args as add_card_args
...
def main(args: argparse.Namespace) -> None:
    ...
    card = CardController(student_model, args)
    if args.card_enable:
        n_prunable = len(card.pruner.convs) if card.pruner is not None else 0
        print(
            f"[Stage 2][CARD] enabled: prune_target={args.card_prune_target} "
            f"quant_bits={args.card_quant_bits} quant_target_ratio={args.card_quant_target_ratio} "
            f"delay={args.card_delay_epochs}ep warmup={args.card_warmup_epochs}ep prunable_layers={n_prunable}",
            flush=True,
        )

    global_step = 0
    optimizer_step = 0  # counts only batches that actually update the student
    try:
        for epoch in range(args.epochs):
            ...
            for batch_i, (weak_imgs, strong_imgs, _paths, weak_infos, strong_infos) in enumerate(dataloader, start=1):
                ...
                # CARD-v2 MUST be prepared before the current student forward.
                # The reliability measured from the current teacher pseudo-labels
                # therefore controls the current amount of pruning/fake-QAT.
                card_stats = card.prepare_step(optimizer_step, len(dataloader), avg_conf)
                ...
                if args.print_freq > 0 and (
                    ...
                    card_str = (
                        f" card_sparsity={card_stats['card_sparsity']:.3f} "
                        f"card_quant_ratio={card_stats['card_quant_ratio']:.3f}"
                        if card_stats
                        else ""
                    )
                    print(
                        ...
                        f"lr={optimizer.param_groups[0]['lr']:.6g}{card_str}",
                        flush=True,
                    )
            ...
            card_summary = f" card_sparsity={card.pruner.sparsity():.3f}" if args.card_enable else ""
            print(
                ...
                f"dfl={sums['dfl'] / denom:.4f} mard={sums['mard'] / denom:.4f}{card_summary}",
                flush=True,
            )
            ...
            if save_this_epoch:
                ckpt_path = checkpoint_dir / f"yolo26_stage2_card_v2_epoch_{epoch + 1}.pt"
                student_wrapper.model = student_model
                # Monkey-patched forwards are training runtime state, not part of
                # the dense YOLO checkpoint. Save them separately and reinstall.
                with card.suspended_for_serialization():
                    student_wrapper.save(str(ckpt_path))
                card_state_path = ckpt_path.with_suffix(".card_state.pt")
                card.save_state(card_state_path)
                print(
                    f"[Epoch {epoch + 1:03d}/{args.epochs:03d}] saved dense-latent student: {ckpt_path} "
                    f"+ CARD state: {card_state_path}",
                    flush=True,
                )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="RT-SFOD Stage 2 + optional CARD-v2 reliability-guided student compression")
