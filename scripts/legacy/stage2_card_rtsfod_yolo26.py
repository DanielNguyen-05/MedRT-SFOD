"""Stage 2 RT-SFOD-Lite adaptation for YOLO26 end-to-end detectors.

RT-SFOD-Lite "Try 2": identical to stage2_rtsfod_yolo26.py (mean-teacher
self-training, DHF, MARD — unchanged, defaults match Table S.1/S.2 of the
RT-SFOD paper) plus CARD (Compression-Aware Representation Diversification,
see card_compression.py): the student is progressively channel-pruned and
quantization-aware-trained *during* self-training, gated by pseudo-label
confidence the same way MARD's lambda(t) is. The teacher is left untouched
(full precision, dense) and continues to be the stable pseudo-label source.

Enable with --card_enable; all CARD flags default to inert values otherwise,
so this script is a strict superset of stage2_rtsfod_yolo26.py and can be run
with --card_enable omitted to reproduce the original behaviour exactly.
"""
...
from scripts.legacy.card_compression import CardController, add_card_args
...
def main(args: argparse.Namespace) -> None:
    ...
    if args.card_enable:
        ...
        print(
            ...
            f"warmup={args.card_warmup_epochs}ep prunable_layers={n_prunable}",
            flush=True,
        )
    ...
    try:
        for epoch in range(args.epochs):
            ...
            for batch_i, (weak_imgs, strong_imgs, _paths, weak_infos, strong_infos) in enumerate(dataloader, start=1):
                ...
                card_stats = card.step(global_step, len(dataloader), avg_conf)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="RT-SFOD-Lite Stage 2 (DHF + MARD + optional CARD) for YOLO26")
